...
class PortfolioOptimzer:
    ...
    def __no_shrinkage(self, corr_matrix):
        return corr_matrix
    
    def __linear_shrinkage(self, corr_matrix, alpha=0.5): 
        '''Method of A'''
        return  alpha * np.identity(corr_matrix.shape[0]) + (1-alpha) * corr_matrix
    
    def __constant_correlation_model(self, corr_matrix): 
        '''Method B'''
        n = len(corr_matrix)
    ...
    def __eigenvalue_clipping(self, corr_matrix, k=1): 
        '''Method C'''
    ...
    def __kmeans_clustering(self, corr_matrix, alpha=0.5):
    ...
    @staticmethod
    def run_optimizer(self, start_year:str, end_year:str, rebalancing:str, shrinkage_method="None"):
        '''
        포트폴리오 최적화를 수행합니다
